...
import com.alan.clients.clickgui.components.IComponent;
...
import com.alan.clients.utility.profile.Manager;
import com.alan.clients.utility.profile.ProfileModule;
...
public class ModuleComponent implements IComponent {
    ...
    public void render() {
        ...
        if (this.mod.moduleCategory() == Module.category.profiles && !(this.mod instanceof Manager) && !((ProfileModule) this.mod).saved && Rise.currentProfile.getModule() == this.mod) {
    ...
    public void onDrawScreen(int x, int y) {
        ...
        if (hovering && categoryComponent.isCategoryOpened() && Gui.toolTip.isToggled() && mod.getPrettyToolTip() != null) {
            Rise.clickGui.run(() -> RenderUtils.drawToolTip(mod.getPrettyToolTip(), x, y));
        }
    }
        ...
        if (this.isHover(x, y) && b == 0 && this.mod.canBeEnabled()) {
            ...
            if (this.mod.moduleCategory() != Module.category.profiles) {
                if (Rise.currentProfile != null) {
                    ((ProfileModule) Rise.currentProfile.getModule()).saved = false;
                }
            }
        }
