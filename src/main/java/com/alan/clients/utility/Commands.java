package com.alan.clients.utility;
...
import com.alan.clients.module.impl.minigames.DuelsStats;
import com.alan.clients.module.impl.other.FakeChat;
import com.alan.clients.module.impl.other.KillMessage;
import com.alan.clients.module.impl.other.NameHider;
    ...
    public static void rCMD(@NotNull String c) {
        if (!c.isEmpty()) {
            ...
            if (firstArg.equals("setkey")) {
                ...
                File folder = new File(Rise.mc.mcDataDir, "keystrokes");
                try {
                ...
                Rise.getExecutor().execute(AutoUpdate::update);
                ...
                Rise.debugger = !Rise.debugger;
                print("Debug " + (Rise.debugger ? "enabled" : "disabled") + ".", 1);
            } else if (firstArg.equals("profiles") || firstArg.equals("p")) {
                if (!hasArgs) {
                    print("&aAvailable profiles:", 1);
                    if (Rise.profileManager.profiles.isEmpty()) {
                        print("None", 0);
                        return;
                    }
                    for (int i = 0; i < Rise.profileManager.profiles.size(); ++i) {
                        print(i + 1 + ". " + Rise.profileManager.profiles.get(i).getName(), 0);
                    }
                    ...
                    for (Profile profile : Rise.profileManager.profiles) {
                        if (profile.getName().equals(name)) {
                            Rise.profileManager.loadProfile(profile.getName());
                    ...
                    for (Profile profile : Rise.profileManager.profiles) {
                        if (profile.getName().equals(name)) {
                            Rise.profileManager.deleteProfile(profile.getName());
