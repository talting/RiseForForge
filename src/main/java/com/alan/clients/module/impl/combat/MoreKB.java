        ...
        switch ((int) mode.getInput()) {
            case 0:
                if (sprintReset.isToggled()) {
                    ...
                    Rise.getExecutor().schedule(() -> KeyBinding.setKeyBindState(mc.gameSettings.keyBindForward.getKeyCode(), Keyboard.isKeyDown(mc.gameSettings.keyBindForward.getKeyCode())),
                            (long) rePressDelay.getInput(), TimeUnit.MILLISECONDS);
                }
                if (sneak.isToggled()) {
                    KeyBinding.setKeyBindState(mc.gameSettings.keyBindSneak.getKeyCode(), true);
                    Rise.getExecutor().schedule(() -> KeyBinding.setKeyBindState(mc.gameSettings.keyBindSneak.getKeyCode(), Keyboard.isKeyDown(mc.gameSettings.keyBindSneak.getKeyCode())),
                            (long) rePressDelay.getInput(), TimeUnit.MILLISECONDS);
                }
                break;
            case 1:
