...
public class KillAura extends IAutoClicker {
        ...
        if (block.get() && (input == 3 || input == 4 || input == 5 || input == 9) && Utils.holdingSword()) {
            ...
            switch (input) {
                case 3:
                    if (lag) {
                        blinking = true;
                        if (Rise.badPacketsHandler.playerSlot != mc.thePlayer.inventory.currentItem % 8 + 1) {
                            mc.thePlayer.sendQueue.addToSendQueue(new C09PacketHeldItemChange( mc.thePlayer.inventory.currentItem % 8 + 1));
                            Rise.badPacketsHandler.playerSlot = mc.thePlayer.inventory.currentItem % 8 + 1;
                            swapped = true;
                        }
                        lag = false;
                    } else {
                        ...
                        Rise.badPacketsHandler.playerSlot = mc.thePlayer.inventory.currentItem;  // todo recode this with slot handler
                        swapped = false;
    ...
    private void setBlockState(boolean state, boolean sendBlock, boolean sendUnBlock) {
        if (Utils.holdingSword()) {
            if (sendBlock && !blocking && state && Utils.holdingSword() && !Rise.badPacketsHandler.C07) {
                sendBlock();
    ...
    public void resetBlinkState(boolean unblock) {
        ...
        releasePackets();
        blocking = false;
        if (Rise.badPacketsHandler.playerSlot != mc.thePlayer.inventory.currentItem && swapped) {
            mc.thePlayer.sendQueue.addToSendQueue(new C09PacketHeldItemChange(mc.thePlayer.inventory.currentItem));
            Rise.badPacketsHandler.playerSlot = mc.thePlayer.inventory.currentItem;
            swapped = false;
        }
        if (lag && unblock) {
            unBlock();
            ...
            synchronized (blinkedPackets) {
                for (Packet<?> packet : blinkedPackets) {
                    if (packet instanceof C09PacketHeldItemChange) {
                        Rise.badPacketsHandler.playerSlot = ((C09PacketHeldItemChange) packet).getSlotId();
                    }
