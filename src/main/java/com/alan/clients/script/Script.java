...
public class Script {
    ...
    public boolean run() {
        try {
            ...
            final File file = new File(Rise.scriptManager.tempDir);
            ...
            final StandardJavaFileManager standardFileManager = Rise.scriptManager.compiler.getStandardFileManager(bp, null, null);
            final ArrayList<String> list = new ArrayList<>();
            list.add("-d");
            list.add(Rise.scriptManager.tempDir);
            list.add("-XDuseUnsharedTable");
            if (!(boolean) Launch.blackboard.get("fml.deobfuscatedEnvironment")) {
                list.add("-classpath");
                String s = Rise.scriptManager.b;
                try {
            ...
            boolean success = Rise.scriptManager.compiler.getTask(null, standardFileManager, bp, list, null, Collections.singletonList(new ClassObject(this.scriptName, this.codeStr, this.extraLines))).call();
            if (!success) {
                this.error = true;
                return false;
            }
    ...
    public void delete() {
        this.d = null;
        this.b = null;
        final File file = new File(Rise.scriptManager.tempDir + File.separator + this.scriptName + ".class");
        if (file.exists()) {
            file.delete();
        }
    }
    ...
    public void createScript(final String s) {
        ...
        final Iterator<String> iterator = Rise.scriptManager.imports.iterator();
        while (iterator.hasNext()) {
            extraLines++;
            sb.append("import ").append(iterator.next()).append(";\n");
        }
        sb.append("import com.alan.clients.script.classes.*;\n");
        sb.append("import com.alan.clients.script.packets.clientbound.*;\n");
        sb.append("import com.alan.clients.script.packets.serverbound.*;\n");
